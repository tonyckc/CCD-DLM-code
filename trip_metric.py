...
def compute_score(
    cities: list[str], durations: list[str], responses: list[str],return_preds=False,
):
    ...
    if return_preds:
        ...
        return hard_acc,[i for i, j in enumerate(hard_scores) if j == 1.0]

    hard_acc = sum(hard_scores) / len(hard_scores)
    return hard_acc

def trip_metric(data, preds,return_preds=False):
    ...
    for item, pred in zip(data.values(), preds):
        ...
        sample_count += 1
    if return_preds:
        hard_acc, scores = compute_score(cities, durations, responses, return_preds=return_preds)
